...
// ------------------------
      ...
      //Create an auctioneer and two participants
      createAgent("Auctioneer", home, "Auctioneer");
      createAgent("HeritageMalta", container[0],"Participant");
      createAgent("MuseoGalileo", container[0],"Participant");
      
      //Clone agents and move it to container where auction will take place
      cloneAgent("Auctioneer", container[1], "Auctioneer-clone1");
      cloneAgent("HeritageMalta", container[1], "HeritageMalta-clone1");
      cloneAgent("MuseoGalileo", container[1], "MuseoGalileo-clone1");
      
    //Clone agents and move it to container where auction will take place
      //cloneAgent("Auctioneer", container[2], "Auctioneer-clone2");
      //cloneAgent("HeritageMalta", container[2], "HeritageMalta-clone2");
      //cloneAgent("MuseoGalileo", container[2], "MuseoGalileo-clone2");
   }
...
// ----------------------------------------
	  ...
	  if (command == NEW_AGENT) {
         ...
         try {
            ...
            a = home.createNewAgent(name, ParticipantMobileAgent.class.getName(), args);
	        a.start();
	        agents.add(name);
   ...
   void createAgent(String name, AgentContainer container, String agentType){

	     jade.wrapper.AgentController a = null;
       try {
          Object[] args = new Object[2];
          args[0] = getAID();
          if(agentType.equals("Participant")) a = container.createNewAgent(name, ParticipantMobileAgent.class.getName(), args);
          else if(agentType.equals("Auctioneer")) a = container.createNewAgent(name, AuctioneerMobileAgent.class.getName(), args);
	      a.start();
	      agents.add(name);
	      myGui.updateList(agents);
	     }
   ...
   void cloneAgent(String name, AgentContainer container, String newName){
	   
	   AID aid = new AID(name, AID.ISLOCALNAME);
		try {
		    String destName = container.getContainerName();
			Location dest = (Location)locations.get(destName);
	        MobileAgentDescription mad = new MobileAgentDescription();
	        mad.setName(aid);
	        mad.setDestination(dest);
